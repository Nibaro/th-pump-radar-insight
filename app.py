...
def fetch_data():
    ...
    try:
        ...
        if isinstance(data, dict) and 'features' in data:
            ...
            # --- ปรับปรุงการหาคอลัมน์ให้ปลอดภัยขึ้น ---
            name_c = next((c for c in df.columns if 'name' in c or 'station' in c), "station_name")
            if "station_name" not in df.columns and name_c == "station_name":
                df["station_name"] = "ไม่ระบุชื่อสถานี"
            ...
            def map_status(s):
                ...
                return '⚪ ไม่ทราบสถานะ'
            df['status_group'] = df[status_c].apply(map_status)
...
if not df_raw.empty:
    ...
    with tab5:
        ...
        if not df_final.empty and fuel_col in df_final.columns:
            ...
            # --- แก้ไขจุด KeyError โดยระบุชื่อคอลัมน์ให้ชัดเจน ---
