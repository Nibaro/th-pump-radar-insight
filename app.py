...
if not df_raw.empty:
    ...
    with st.expander("📄 **บทสรุปผู้บริหาร (Executive Summary)**", expanded=True):
        ...
        พบสถานีบริการน้ำมันที่พร้อมให้บริการ **{len(df_final)} แห่ง** และพบปัญหาน้ำมันหมดจำนวน **{out_of_stock} แห่ง** ซึ่งอาจส่งผลกระทบต่อภารกิจในพื้นที่ **{sel_subdist if sel_subdist != 'ทั้งหมด' else 'โครงการฯ'}**
        """)

    with st.expander("💡 **ข้อเสนอแนะเชิงนโยบาย (Policy Recommendations)**"):
        st.write("""
        1. **มาตรการความต่อเนื่อง:** กรณีน้ำมันขาดแคลนในพื้นที่เป้าหมายเกิน 20% ให้เจ้าหน้าที่เตรียมน้ำมันสำรองก่อนเข้าพื้นที่
        2. **การวางแผนเส้นทาง:** ใช้ข้อมูล Real-time นี้ในการกำหนดจุดเติมน้ำมันระหว่างทางเพื่อลดความเสี่ยงรถติดหล่มพลังงาน
        3. **การรายงาน:** หากพบปั๊มน้ำมันหมดในพื้นที่อนุรักษ์ฯ ต่อเนื่อง ควรรายงานเข้าส่วนกลางเพื่อประสานงานพลังงานจังหวัด
        """)
