...
if not df_raw.empty:
    ...
    with tab1:
        ...
        if not df_final.empty:
            ...
            for _, row in df_final.iterrows():
                ...
                folium.Marker([row['latitude'], row['longitude']], popup=f"<b>{row[name_col]}</b>", icon=folium.Icon(color=color, icon='gas-pump', prefix='fa')).add_to(cluster)
            folium_static(m, width=1100)

    with tab5:
        st.subheader("⛽ สถิติแยกประเภทน้ำมัน")
    ...
    st.sidebar.markdown("---")
    st.sidebar.info("""**จัดทำโดย:** แผนกวิชาการ โครงการอนุรักษ์พันธุกรรมพืชฯ (อพ.สธ.)
